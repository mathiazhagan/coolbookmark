        ...
        try {
            ...
            for (int i = 0; i < temp.length; i++) {
                if (checkurl.exists(temp[i])) {
                    Criteria dup = myDao.getDbsession().createCriteria(Bookmarks.class);
                    dup.add(Restrictions.eq("bookmarkUrl", temp[i]));
                    dup.setMaxResults(1);
                    if (dup.list().isEmpty()) {
                        ...
                        bk.setStatus(userEnum.publics.getUserType());
            ...
            Scribbles urfeed = new Scribbles(user, desc,userEnum.bookmarks.getUserType(),"PUBLIC", date);
